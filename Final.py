...
def handle_claim_territory(game: Game, bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    """At the start of the game, claim unclaimed territories in preferred sequences."""

    unclaimed_territories = game.state.get_territories_owned_by(None)
    my_territories = game.state.get_territories_owned_by(game.state.me.player_id)

    # Function to count adjacent friendly territories
    def count_adjacent_friendly(x: int) -> int:
        return len(set(my_territories) & set(game.state.map.get_adjacent_to(x)))

    # Define preferred sequences
    preferred_sequences = [
        range(28, 32),  # Second preference: 28-31
        range(38, 42),  # First preference: 38-41
        range(32, 38)   # Third preference: 32-37
    ]

    # Attempt to claim territories in preferred sequences
    for sequence in preferred_sequences:
        available_in_sequence = [x for x in unclaimed_territories if x in sequence]
        if available_in_sequence:
            # Sort by most adjacent friendly territories and claim the best one
            selected_territory = sorted(
                available_in_sequence,
                key=lambda x: count_adjacent_friendly(x),
                reverse=True
            )[0]
            break
    else:
        # If no preferred sequence is available, claim territory with the most adjacent territories
        selected_territory = sorted(
    ...
    # Attempt to claim adjacent territories if available
    adjacent_to_selected = game.state.map.get_adjacent_to(selected_territory)
    adjacent_available = [x for x in adjacent_to_selected if x in unclaimed_territories]

    if adjacent_available:
        adjacent_territory = sorted(
            adjacent_available,
            key=lambda x: count_adjacent_friendly(x),
            reverse=True
        )[0]
        selected_territory = adjacent_territory
...
def handle_place_initial_troop(game: Game, bot_state: BotState, query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    """Place initial troop strategically after all territories have been claimed."""

    # Function to count adjacent friendly troops
    def count_adjacent_friendly_troops(territory_id: int) -> int:
        adjacent_territories = game.state.map.get_adjacent_to(territory_id)
        return sum(game.state.territories[adj].troops for adj in adjacent_territories if adj in game.state.territories and game.state.territories[adj].player_id == game.state.me.player_id)

    # Get all owned territories and border territories
    my_territories = game.state.get_territories_owned_by(game.state.me.player_id)
    border_territories = game.state.get_all_border_territories(my_territories)

    # Prioritize placing troops on territories in the specified range (32-37)
    preferred_range = list(range(28, 32))

    # Check if any preferred range territories are available
    available_preferred = [territory_id for territory_id in preferred_range if territory_id in my_territories]
    if available_preferred:
        return game.move_place_initial_troop(query, random.choice(available_preferred))

    # If no preferred range territories are available, place on any owned territory
    available_non_preferred = [territory_id for territory_id in my_territories if territory_id not in preferred_range]
    if available_non_preferred:
        return game.move_place_initial_troop(query, random.choice(available_non_preferred))

    # If no preferred range territories are available, place on the border territory with the most adjacent friendly troops
    max_adjacent_troops_territory = max(border_territories, key=lambda x: count_adjacent_friendly_troops(x))

    return game.move_place_initial_troop(query, max_adjacent_troops_territory)
...
def handle_attack(game: Game, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    """After the troop phase of your turn, prioritize attacking based on strategic considerations."""

    # Function to determine if we can attack a territory
    def can_attack(attacker_id: int, defender_id: int) -> bool:
        return game.state.territories[attacker_id].troops > 1.3 * game.state.territories[defender_id].troops
    ...
    continent_ranges = {
        "North America": range(0, 9),
        "Europe": range(9, 16),
        "Asia": range(16, 28),
        "South America": range(28, 32),
        "Africa": range(32, 38),
        "Oceania": range(38, 42),
    }

    # Function to attack weakest territory within a range
    def attack_weakest_in_range(territories: list[int], territory_range: range) -> Optional[MoveAttack]:
        # Filter territories within the specified range
        territories_in_range = [t for t in territories if t in territory_range]
        return attack_weakest(territories_in_range)

    def attack_weakest(territories: list[int]) -> Optional[MoveAttack]:
        # Sort territories by weakest
    ...
    if len(game.state.recording) < 4000:
        # Check recent attacks against us
        new_records = game.state.recording[game.state.new_records:]
        enemy = None
        for record in new_records:
            if isinstance(record, MoveAttack) and record.defending_territory in set(my_territories):
                enemy = record.move_by_player
                break

        # Set enemy based on recent attacks or weakest adjacent territory
        if enemy is not None:
            if bot_state.enemy is None or random.random() < 0.05:
        ...
        # Prioritize attacking within specific continents
        for continent, territory_range in continent_ranges.items():
            enemy_territories = list(set(bordering_territories) & set(game.state.get_territories_owned_by(bot_state.enemy)))
            move = attack_weakest_in_range(enemy_territories, territory_range)
            if move is not None:
                return move

        # Otherwise, attack anyone with favorable battle most of the time
        ...
        # Late game: prioritize attacking within specific continents
        for continent, territory_range in continent_ranges.items():
            ...
            for territory in strongest_territories:
                adjacent_enemy_territories = list(set(game.state.map.get_adjacent_to(territory)) - set(my_territories))
                move = attack_weakest_in_range(adjacent_enemy_territories, territory_range)
                if move is not None:
                    return move
